...
//MARK: - Protocol
//у всех автомобилей общее - наличие бренда, года выпуска, объёма багажника, также наличие изменяемых вариаций: состояние дверей, окон, двигателя
protocol Car: class {
...
//в расширения протокола выносим отдельными бусинками открыть двери, открыть окна, завести / заглушить двигатель
...
class TrunkCar: Car {
    ...
    init (brand: String, releaseYear: Int, windowsState: WindowsState, doorsState: DoorsState, engineState: EngineState, trunkState: Bool, trunkVolume: Double, trunkVolumeFilled: Double) {
        self.brand = brand
...
extension TrunkCar: CustomStringConvertible {
    var description: String {
        return "Автомобиль \(self.brand), \(self.releaseYear), с багажником на \(self.trunkVolume) кг. Сейчас \(self.windowsState.rawValue), \(self.doorsState.rawValue), \(self.engineState.rawValue), багажник \(self.trunkState == true ? "прицеплен" : "отцеплен") и \(self.trunkVolumeFilled > 0 ? "загружен на \(self.trunkVolumeFilled) кг" : "не загружен")."
    }
}
...
class SportCar: Car {
    ...
    init (brand: String, releaseYear: Int, windowsState: WindowsState, doorsState: DoorsState, engineState: EngineState, roofState: RoofState, gasKilometrage: Double) {
        self.brand = brand
        self.releaseYear = releaseYear
        self.windowsState = windowsState
        self.doorsState = doorsState
...
extension SportCar: CustomStringConvertible {
    var description: String {
        return "Автомобиль-кабриолет \(self.brand), \(self.releaseYear), расход топлива \(self.gasKilometrage) на 100 км. Сейчас \(self.windowsState.rawValue), \(self.doorsState.rawValue), \(self.engineState.rawValue), \(self.roofState.rawValue). "
    }
}

//MARK: - Create objects, make actions

var trunkСar1 = TrunkCar (brand: "Jeep", releaseYear: 2010, windowsState: .open, doorsState: .open, engineState: .turnedOn, trunkState: true, trunkVolume: 2000.0, trunkVolumeFilled: 311.0)
var trunkCar2 = TrunkCar (brand: "Toyota", releaseYear: 2000, windowsState: .closed, doorsState: .closed, engineState: .turnedOff, trunkState: false, trunkVolume: 2100.0, trunkVolumeFilled: 0.0)

var sportCar1 = SportCar (brand: "Subaru", releaseYear: 2005,  windowsState: .closed, doorsState: .open, engineState: .turnedOn, roofState: .lowered, gasKilometrage: 10.0)
var sportCar2 = SportCar (brand: "Ferrary", releaseYear: 1990, windowsState: .closed, doorsState: .open, engineState: .turnedOff, roofState: .lowered, gasKilometrage: 12.0)
